...
def audio_info_extraction_serial(audio_info_df, audio_info_df_fn, new_columns, praat_parameters, dest_paths):
    ...
    praat_fn_cols = [(x[0], x[1].params, x[1].pitch, x[1].pitch_tier, x[1].point_process, x[1].intensity, x[1].intensity_tier, x[1].voice_report) for x in audio_pfp_list]
...
def extract_audio_informations(audio_info_df_fn, praat_parameters, dest_paths, parallel=False, n_jobs=-1):
    ...
    new_columns = ["complete_event_file", "parameters_file", "pitch_file", "pitch_tier_file", "point_process_file", "intensity_file", "intensity_tier_file", "voice_report_file"]
    if parallel:
